...
The working repository for the code is

	https://github.com/efabless/caravel_openframe_project

This directory contains run-time files from synthesis, place, and route,
and is 2GB.  The "clean" version of this repository (no run-time files)
is
	https://github.com/RTimothyEdwards/caravel_openframe_project

In the second repository, branch "test" contains the testbench code in
...
The example project was submitted on a ChipIgnite run prior to the
public release of the Openframe version as a standard submission
method, as a proof-of-concept and reference application.

Silicon testing began in late 2024, with a reasonably comprehensive
set of tests run in January 2025.

Tests are currently in the "test" branch of the repository at
https://github.com/RTimothyEdwards/caravel_openframe_project, found
in directory verilog/dv/.  Test directories are:

	blink --- Basic LED blink test.  Note that unlike Caravel,
		the GPIO pin on the openframe that drives the LED
		on the board is just another generic GPIO resource,
		and needs to be treated accordingly.

		Tested and works.

	gpio_vector --- Because all GPIOs on this openframe project
		example are independently configured via wishbone
		bus interface instead of the serial shift register
		used on caravel/caravan, a separate interface is
		needed to simultaneously set or read the state of
		multiple GPIOs at the same time.

		Tested and works.

	qspi --- Unlike the VexRISC, the PicoRV32 is able to
		access the flash SPI in single, dual, and quad
		modes, DDR, and CRM, with the ability to switch
		between modes on demand.

		Tested and fails at standard corner/voltage/temperature.
		Issue appears to be timing with respect to the "jump" 
		wire, which also was a problem with MPW-one, and may
		be a problem with misconfiguration of the timing
		constraints, as it does not show up in timing analysis.
		The observation is that after switching to any mode
		other than single SPI, the SPI gets addressed through
		a jump statement but continues without attempting to
		change the address, until the processor gets misaligned
		with the data and enters the trap state.

		Tested also at vccd supply ranging from 1.55V to 2.2V
		with no change in behavior.  Tested with clock speeds
		down to 1MHz with no change in behavior.

	sram --- Unlike the caraven/caravel processor, this project
		reverts to the original openRAM SRAM block used on
		the MPW-one Caravel version.  This testbench checks
		reading/writing memory.

		Tested and works (note that the SRAM is known not
		to operate at some extreme corners;  this test has
		only been run at standard corner/voltage/temperature).

	uart --- The UART in this project is more capable than the
		VexRISC UART, and has configuration control of the
		UART frequency, so it can be adjusted to retain the
		same baud rate relative to the core clock.
