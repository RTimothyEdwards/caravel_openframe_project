...
#define reg_sram_block(i) (*(volatile uint32_t*)(0x00000000 + (i)))

// --------------------------------------------------------
// Caravel openframe project --- PicoRV32
//
// Run a basic test of SRAM memory.
// Writes and reads memory locations continously, prints
// results to the UART.
// --------------------------------------------------------
...
{
    int i, j;
    uint32_t mem_addr, mem_data, mem_write_data;
	...
	mem_data = reg_sram_block(mem_addr);
        print_hex(mem_addr, 4);
        print(" = ");
        print_hex(mem_data, 8);
        // print("\n");
        print(" ");
	...
	reg_sram_block(mem_addr) = mem_addr + mem_write_data;

        mem_addr += 4;
        if (mem_addr >= 900) mem_addr = 100;
