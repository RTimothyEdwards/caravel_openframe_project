...
void main(){
    for (int i = 0; i < 36; i++){
        GPIO_Configure(i, GPIO_MODE_VECTOR_INPUT_PULLUP);
    }
    GPIO_Configure(43, GPIO_MODE_VECTOR_INPUT_PULLUP);
    
    // low
    wait_over_input_l(0xAA,0x3FFFFFFF);
    wait_over_input_l(0XBB,0x0);
    wait_over_input_l(0XCC,0x2AAAAAAA);
    wait_over_input_l(0XDD,0x15555555);
